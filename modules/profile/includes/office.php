...
echo '' .
    ...
    '<div>' . $asset->img('images/contacts.png')->class('icon') . '<a href="index.php">' . _t('Profile') . '</a></div>' .
    '<div>' . $asset->img('images/rate.gif')->class('icon') . '<a href="?act=stat">' . _t('Statistics') . '</a></div>' .
    '<div>' . $asset->img('images/photo.gif')->class('icon') . '<a href="../album/index.php?act=list">' . _t('Photo Album') . '</a>&#160;(' . $total_photo . ')</div>' .
    '<div>' . $asset->img('images/guestbook.gif')->class('icon') . '<a href="?act=guestbook">' . _t('Guestbook') . '</a>&#160;(' . $user['comm_count'] . ')</div>';

if ($systemUser->rights >= 1) {
    $guest = $container->get('counters')->guestbook(2);
    echo '<div>' . $asset->img('images/forbidden.png')->class('icon') . '<a href="../guestbook/index.php?act=ga&amp;do=set">' . _t('Admin-Club') . '</a> (<span class="red">' . $guest . '</span>)</div>';
}
echo '</p></div>';
...
echo '<div>' . $asset->img('images/mail-inbox.png')->class('icon') . '<a href="../mail/index.php?act=input">' . _t('Received') . '</a>&nbsp;(' . $count_input . ($new_mail ? '/<span class="red">+' . $new_mail . '</span>' : '') . ')</div>';
...
echo '<div>' . $asset->img('images/mail-send.png')->class('icon') . '<a href="../mail/index.php?act=output">' . _t('Sent') . '</a>&nbsp;(' . $count_output . ($count_output_new ? '/<span class="red">+' . $count_output_new . '</span>' : '') . ')</div>';
...
echo '<div>' . $asset->img('images/mail-info.png')->class('icon') . '<a href="../mail/index.php?act=systems">' . _t('System') . '</a>&nbsp;(' . $count_systems . ($count_systems_new ? '/<span class="red">+' . $count_systems_new . '</span>' : '') . ')</div>';
...
echo '<div>' . $asset->img('images/file.gif')->class('icon') . '<a href="../mail/index.php?act=files">' . _t('Files') . '</a>&nbsp;(' . $count_file . ')</div>';
...
echo '<div>' . $asset->img('images/user.png')->class('icon') . '<a href="../mail/">' . _t('Contacts') . '</a>&nbsp;(' . $count_contacts . ')</div>';

//Заблокированные
$count_ignor = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='" . $systemUser->id . "' AND `ban`='1'")->fetchColumn();
echo '<div>' . $asset->img('images/user-block.png')->class('icon') . '<a href="../mail/index.php?act=ignor">' . _t('Blocked') . '</a>&nbsp;(' . $count_ignor . ')</div>';
echo '</p></div>';

// Блок настроек
echo '<div class="bmenu"><p><h3>' . _t('Settings') . '</h3>' .
    '<div>' . $asset->img('images/user-edit.png')->class('icon') . '<a href="?act=edit">' . _t('Edit Profile') . '</a></div>' .
    '<div>' . $asset->img('images/lock.png')->class('icon') . '<a href="?act=password">' . _t('Change Password') . '</a></div>' .
    '<div>' . $asset->img('images/settings.png')->class('icon') . '<a href="?act=settings">' . _t('System Settings') . '</a></div>';

if ($systemUser->rights >= 6) {
    echo '<div>' . $asset->img('images/forbidden.png')->class('icon') . '<span class="red"><a href="../admin/"><b>' . _t('Admin Panel') . '</b></a></span></div>';
}
echo '</p></div>';

// Выход с сайта
echo '<div class="rmenu"><p><a href="' . $config['homeurl'] . '/login/">' . $asset->img('images/del.png')->class('icon') . _t('Exit') . '</a></p></div>';

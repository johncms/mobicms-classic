...
if ($total) {
    $req = $db->query("SELECT * FROM `cms_users_iphistory` WHERE `user_id` = '" . $user['id'] . "' ORDER BY `time` DESC" . $tools->getPgStart(true));
    $i = 0;
...
if ($total > $userConfig->kmess) {
    echo '<p>' . $tools->displayPagination('?act=ip&amp;user=' . $user['id'] . '&amp;', $total) . '</p>';

...
if (!$tools->isIgnor($id) && empty($systemUser->ban['1']) && empty($systemUser->ban['3'])) {
    ...
    $out .= '<div class="gmenu">' .
        ...
        '<p>';
    $out .= $container->get(Mobicms\Api\BbcodeInterface::class)->buttons('form', 'text');
    $out .= '<textarea rows="' . $userConfig->fieldHeight . '" name="text"></textarea></p>';
...
if ($id) {
    ...
    if ($total) {
        ...
        if ($total > $userConfig->kmess) {
            $out .= '<div class="topmenu">' . $tools->displayPagination('index.php?act=write&amp;id=' . $id . '&amp;', $total) . '</div>';
        }
